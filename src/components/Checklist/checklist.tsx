...
export const ChecklistItens = (props: ChecklistItensProps) => {
  ...
  const renderRightActions = () => {
    return (
      <Pressable
        className="w-32 h-36 mt-1 justify-center items-center rounded-lg bg-red-800"
        onPress={deleteItem}>
  ...
  return (
    <GestureHandlerRootView className="w-full">
      <Swipeable renderRightActions={renderRightActions}>
        <Pressable
          className="relative w-full px-2 h-36 justify-center bg-white shadow-lg my-1 rounded-lg"
          onPress={editItem}>
          ...
          <View className="flex-row items-center mb-2">
            ...
            <Text
              className={`ml-2 ${isCheck ? "line-through" : ""}`}
              numberOfLines={2}>
              {text}
            </Text>
          </View>
